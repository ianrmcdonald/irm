---
title: Oregon Precincts and the 2020 Election
author: Ian McDonald
date: '2021-03-12'
slug: []
categories:
...
#Assigns a default of "true" for echo in code chunks.
...
```{r Centering Address, include=FALSE}
...
```{r County shapefiles, include=FALSE}
...
The **usmap** package provides a fips_info function that generates county names in a fips table from a single column of FIPS input.  This chunk attaches names to the county shapefile.

```{r Look up FIPS codes, include=FALSE}
...
```{r Create a state boundary layer and a bounding box for leaflet maps, include=FALSE}
...
```{r Generate a sample map of county shapes, include=FALSE}
...
```{r Precinct shapefiles, echo=FALSE, include=FALSE}
...
```{r Portland Metro county shapefile, include=FALSE}
...
or_precincts_votes <- open_election_format_votes(
    here("static/votes/oregon_precincts.csv"), 
...
lane_votes <- read_csv(here("static/votes/lane.csv"), col_types = cols(.default = "d", precinct = "c")) %>% 
  mutate(county = "Lane") %>% 
...
counties_csv_names <- str_c("here(\"","static/votes/20201103__or__general__", counties, "__precinct.csv\")")

#marion_polk_yamhill_votes <- map_df(counties_csv_names, open_election_format_votes) 
c1 <- open_election_format_votes(here("static/votes/20201103__or__general__marion__precinct.csv"))
c2 <- open_election_format_votes(here("static/votes/20201103__or__general__polk__precinct.csv"))
c3 <- open_election_format_votes(here("static/votes/20201103__or__general__yamhill__precinct.csv"))

marion_polk_yamhill_votes <- bind_rows(c1, c2, c3)
```
...
tillamook_votes <- read_csv(
   here("static/votes/tillamook_votes.csv"), 
...
lookup <- read_csv(here("static/votes_shapefile_lookup.csv"), 
...
multnomah_votes_df <- read_csv(
    here("static/votes/multnomah_votes.csv"), 
...
clackamas_votes_df <- read_csv(
    here("static/votes/clackamas_votes.csv"),
    col_types = cols(.default = "d", precinct = "c")) %>%
  mutate(county = "Clackamas", precinct = str_c("C", precinct))

washington_votes_df <- read_csv(
    here("static/votes/washington_county_votes.csv"), 
    col_types = cols(.default = "d", precinct = "c")) %>% 
...
clark_votes_df <- read_csv(
    here("static/votes/clark_votes.csv"),
    col_types = cols(.default = "d", precinct = "c")) %>% 
