...
```{r echo=TRUE}
m <- read_csv("static/shapefiles/lane.csv")
...
orc <- "counties.shp"
wrc <- "WA_County_Boundaries.shp"


oregon_counties_sf <- st_read("static/shapefiles/oregon_counties/counties.shp", 
  quiet = TRUE) %>% 
  st_transform(or_counties, crs = "NAD83") %>% 
  select(!COUNTY)

washington_state_counties_sf <- st_read(
    "static/shapefiles/washington_counties/WA_County_Boundaries.shp",
    quiet = TRUE) %>%
...
metro_portland_precinct_sf <- st_read(
  "static/shapefiles/metro_portland_precinct/precinct.shp", quiet = TRUE
  ) %>%
...
clark_wa_precincts_sf  <- st_read("static/shapefiles/clark_precinct_shapefiles/Precinct.shp", 
                                  quiet = TRUE) %>%
...
or_precinct_sf <- st_read("static/shapefiles/oregon_precincts/OregonPrecinctsNov2018.shp", quiet = TRUE) %>%
...
or_precincts_votes <- open_election_format_votes(
    "static/votes/oregon_precincts.csv", 
...
lane_votes <- read_csv("static/votes/lane.csv", col_types = cols(.default = "d", precinct = "c")) %>% 
  mutate(county = "Lane") %>% 
...
counties_csv_names <- str_c("static/votes/20201103__or__general__", counties, "__precinct.csv")
...
tillamook_votes <- read_csv(
    "static/votes/tillamook_votes.csv", 
...
lookup <- read_csv("static/votes_shapefile_lookup.csv", 
...
multnomah_votes_df <- read_csv(
    "static/votes/multnomah_votes.csv", 
